<template>
  <div class="flap-card-wrapper" v-show="flapCardVisible">
    <div class="flap-card-bg" :class="{'animation': bounceAnimation}">
  ...
  export default {
    ...
    data() {
      return {
        ...
        bounceAnimation: false, // 登场效果动画控制
        pointList: null,        // 小球数组
        pointAnimation: false   // 控制小球动画
      }
    },
    watch: {
      flapCardVisible(v) {
        if (v) {
          ...
          // 登场动画.3s，执行完登场动画再执行卡牌翻转动画
          setTimeout(() => {
            this.startFlapCardAnimation()
            this.pointAnimation = true
          }, 300)
          // 小球播放动画是.75s(似乎还要多一点?), 到达时间后删除小球的动画样式
          // setTimeout(() => {
